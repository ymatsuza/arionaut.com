...
| `logo-stack.svg` | マーク＋ワードマーク＋タグラインの縦組み（マークの下に `ARIONAUT` ／ オレンジのダッシュを挟んで `EXPLORE, INNOVATE, DISCOVER`）。サイトの hero とヘッダーのブランド表示はこの構成に合わせている |
...
- 星を単独のシンボルとして使わない
- タグライン `EXPLORE, INNOVATE, DISCOVER` は Montserrat 600 / ワードマークの下・オレンジのダッシュ2本に挟んで配置する。
  ダッシュとの間隔は左右16px（`logo-stack.svg` の 560×430 座標系）
- 商号（アリオノート合同会社 / ARIONAUT LLC）はロゴには入れない。社名表記が必要な場面では本文・署名側で併記する
